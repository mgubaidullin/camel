...
/**
 * Unit test for {@link QuteComponent} and {@link QuteEndpoint}
 */

public class QuteComponentTest extends CamelTestSupport {
     ...
     */
    @Test
    public void testQuteWithTemplateHeader() throws Exception {
        // Prepare
        Exchange exchange = createExchangeWithBody("The Body");
        exchange.getIn().setHeader("someHeader", "Some Header");
        exchange.getIn().setHeader(QuteConstants.QUTE_TEMPLATE, "Body='{body}'|SomeHeader='{headers.someHeader}'");
        endSimpleMock.expectedMessageCount(1);
     ...
     */
    @Test
    public void testQuteWithResourceUriHeader() throws Exception {
        // Prepare
        Exchange exchange = createExchangeWithBody("The Body");
        exchange.getIn().setHeader("someHeader", "Some Header");
        exchange.getIn().setHeader(QuteConstants.QUTE_RESOURCE_URI, "/another.txt");
        endSimpleMock.expectedMessageCount(1);
     ...
     */
    @Test
    public void testQuteWithInclude() throws Exception {
        // Prepare
        Exchange exchange = createExchangeWithBody("World");
        exchange.getIn().setHeader(QuteConstants.QUTE_RESOURCE_URI, "/detail.txt");
        endSimpleMock.expectedMessageCount(1);
        endSimpleMock.message(0).body().contains("Start");
        endSimpleMock.message(0).body().contains("Hello World!");
     ...
     */
    @Test
    public void testQutePerformance() throws Exception {
        ...
        assertMockEndpointsSatisfied();
        LoggerFactory.getLogger(getClass()).info("Qute performance: " + stopwatch.taken() + "ms for " + messageCount + " messages");
    }
